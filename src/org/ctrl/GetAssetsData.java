...
public class GetAssetsData extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int[] assets = new int[lenFinances];
            int[] currentAssets = new int[lenFinances];

            String[] dates = new String[lenFinances];
            
            for(int i=0;i<lenFinances;i++){
                Finance f = listFinances.get(i);
                assets[i] = (int) f.getAssets();
                currentAssets[i] = (int) f.getCurrent_assets();
